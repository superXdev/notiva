          ...
          {filteredNotes.length === 0 ? (
            ...
            <div className="space-y-2">
              {filteredNotes.map((note) => (
                ...
                >
                  {/* Folder Path - Only show when not in sidebar */}
                  {!onNoteSelect && note.folderId && (
                  ...
                  <h3 className={cn(
                    ...
                    onNoteSelect ? "text-sm mb-1" : "text-base mb-3"
                  )}>
                    {note.title}
                  </h3>

                  {/* Labels - Simplified for sidebar */}
                  {note.labels.length > 0 && (
                    <div className="flex flex-wrap gap-1 mb-1">
                      {note.labels
                        .slice(0, onNoteSelect ? 2 : 3)
                        .map((labelName) => {
                          ...
                          return (
                            <Badge
                              ...
                              className={cn(
                                "text-xs px-2 py-0.5",
                                onNoteSelect && "text-xs px-1.5 py-0.5"
                              )}
                              style={
                                label
                                  ? {
                      ...
                      {note.labels.length > (onNoteSelect ? 2 : 3) && (
                        <Badge
                          variant="secondary"
                          className={cn(
                            "text-xs px-2 py-0.5",
        ...
        <div className="border-t border-border p-4 flex-shrink-0">
          <div className="flex items-center justify-center space-x-2">
            ...
            <span className="text-sm text-muted-foreground mx-4">
              Page {pagination.notes.page} of {pagination.notes.totalPages}
            </span>
            <Button
              variant="ghost" 
              size="sm"
